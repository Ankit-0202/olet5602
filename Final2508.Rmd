...
```
The expression of SOX2 in the transcriptome starts at a slightly elevated level at 0hr, peaks around 6hr and then begins to steadily decline. The Proteome shows SOX2 starting slighlt postive expression at 0hr, peaks early at 1hr and thens shows similar decline. Both exhibit a general trend of initial upregulation followed by downregulation across time in both the proteome and transcriptome datasets. This suggests that the transcriptonal activtiy might be intially high to kickstart certain cellular proecess but is followed by a reduction as cells progress through differential.
...
Here we train our second model on the down sampled data. The dataset performed best with better with a larger number of trees, and so we have used 1000 for our number of trees. 
...
results <- data.frame(
  ...
  Probability_Class1 = probabilities[, "OSN"],
  Probability_Class2 = probabilities[, "Other"]
)

# Display the table with combined data, actual labels, and probabilities
print(head(results))

# Filter rows where the predicted label is "OSN"
osn_predictions <- results[results$Predicted_Label == "OSN", ]

# Display the filtered results
print(head(osn_predictions))
...
Using `OSN_target_genes` as a comparison with model, suggests that SVN overclassifies target genes. This may suggest a need not for downsampling or it may suggest that there are other methods of dealing with class imabalance. 

```{r}
length(osn_labels)
# Create a vector of all labels predicted as "OSN"
osn_labels <- rownames(osn_predictions)
...
The Random Forest model slightly outperforms the SVM model in identifying OSN genes, with a higher AUC socre of 0.8254 compared to SMV's score of 0.7669. This indicates a stronger overall classification performance. RF correctly identifies more true OSN genes but has a higher rate of false positives. In contrast, SVM has fewer false positives but slightly misses more true oSN genes. The choice between the two should be made based on whether too maximise true positive identification or minimizing false positives.


```{r}
# Make predictions using the tuned Random Forest model
rf_predictions <- predict(tuned_rf_model, newdata = combined_data)

# Create a logical vector for the specific label "OSN"
is_osn_rf <- rf_predictions == "OSN"

length(is_osn_rf)

# Extract row names where the predicted label is "OSN"
osn_rownames_rf <- rownames(combined_data)[is_osn_rf]

# Display the row names
print(osn_rownames_rf)
```


```{r}
# Make predictions using the XGBoost model
xgb_predictions <- predict(xgb_model_tuned, newdata = combined_data)

# Assuming xgb_predictions is a matrix with probabilities
# Convert probabilities to predicted classes
predicted_classes_xgb <- ifelse(xgb_predictions[, "OSN"] > 0.5, "OSN", "Other")

# Create a logical vector for the specific label "OSN"
is_osn_xgb <- predicted_classes_xgb == "OSN"
length(is_osn_xgb)

# Extract row names where the predicted label is "OSN"
osn_rownames_xgb <- rownames(combined_data)[is_osn_xgb]

# Display the row names
print(osn_rownames_xgb)
```

```{r}
# Make predictions using the neural network model
nn_predictions <- predict(nn_model, newdata = combined_data, probability = TRUE)

# Extract the predicted classes
# Assuming nn_predictions is a matrix with probabilities for each class
predicted_classes_nn <- colnames(nn_predictions)[apply(nn_predictions, 1, which.max)]

# Create a logical vector for the specific label "OSN"
is_osn_nn <- predicted_classes_nn == "OSN"
length(is_osn_nn)

# Extract row names where the predicted label is "OSN"
osn_rownames_nn <- rownames(combined_data)[is_osn_nn]

# Display the row names
print(osn_rownames_nn)
```
...
**Methods of mitigating Class Imbalance** - at the moment we have used Ensemble models, and downsampling to mitigate the massive class imbalance. It may be worth looking into other methods, or even not accounting for the imblance and training the models. Our results show clear overclassification of OSN which may be from the downsampling.
